...
public class LoadSearch extends AsyncTask<Void, Void, SearchItem> {
    ...
    public LoadSearch(Context context, SearchLoadedListener myComponent, int page, String query, boolean loadMore) {
    ...
    protected void onPostExecute(SearchItem searchItem) {
        ...
        if (!loadMore)
            dialog.dismiss();
        if (myComponent != null) {
            myComponent.onSearchLoaded(searchItem.getShowsItems(), searchItem.getTotalCount());
        }
    }
}
